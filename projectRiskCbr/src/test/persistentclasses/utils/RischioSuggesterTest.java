...
public class RischioSuggesterTest {
	...
	Collection<RischioSuggester> suggesters;
	...
	public void setUp() {
		...
		suggesters = RischioSuggester.getSuggesters(cases);
	}
	
	@Test
	public void testGetSuggesters() {
		Assert.assertEquals("we should have 2 suggesters, 1 for rischioTypeA and 1 for rischioTypeB", suggesters.size(), 2);
		Iterator<RischioSuggester> iterator = suggesters.iterator();
		Rischio rischio;
		...
		Assert.assertEquals("rischioTypeA suggested costo potenziale impatto",	(1.6 * 75 + 0.4 * 66)/2.0,				rischio.getCostoPotenzialeImpatto(), 0.0);
		...
		Assert.assertEquals("rischioTypeB suggested contingency", 				(0.8 * 43 + 0.8 * 600)/1.6,	rischio.getContingency(), 0.0);
	}
